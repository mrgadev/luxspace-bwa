  ...
  <section class="container mx-auto">
    <div class="flex flex-wrap my-4 md:my-12">
        ...
        <div class="slider">
          ...
          <div class="preview">
            <div class="item rounded-lg h-full overflow-hidden">
              <img
                src="{{Storage::url($product->galleries->first()->url)}}"
                alt="front"
                class="object-cover w-full h-full rounded-lg"
              />
            </div>
      ...
      <div class="flex-1 px-4 md:p-6">
        <h2 class="text-5xl font-semibold mb-3">{{$product->name}}</h2>
        <p class="text-xl">IDR {{number_format($product->price, 0, ',', '.')}}</p>
          ...
          <div class="px-3 flex-none" style="width: 320px">
            <div class="rounded-xl p-4 pb-8 relative bg-white">
              <div class="rounded-xl overflow-hidden card-shadow w-full h-36">
                <img
                  src="{{$recommendation->galleries()->exists() ? Storage::url($recommendation->galleries->first()->url) : ''}}"
                  alt=""
                  class="w-full h-full object-cover object-center"
                />
              </div>
              <h5 class="text-lg font-semibold mt-4">{{$recommendation->name}}</h5>
              <span class="">IDR {{number_format($recommendation->price, 0, '.', ',')}}</span>
